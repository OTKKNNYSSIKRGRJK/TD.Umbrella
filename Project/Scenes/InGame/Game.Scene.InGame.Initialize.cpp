...
namespace Game::Scene::Impl {
						...
						try {
							...
							if (!validMeshes.empty()) {
								...
								if (!diffuseTexName.empty()) {
									// 既存の基本テクスチャ8枚の後に登録される前提でインデックスを計算
									EnemyTextureIndices_[ed.name] = static_cast<uint32_t>(8 + AdditionalTextures_.size());
	...
	void InGame::Initialize() {
		...
		// チュートリアルテクスチャは基本テクスチャ8枚 + 追加テクスチャの直後に配置
		TutorialManager_->TutorialTextureStartIndex = 8U + static_cast<uint32_t>(AdditionalTextures_.size());
