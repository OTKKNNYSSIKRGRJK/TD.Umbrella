...
namespace Game::Scene::Impl {
	...
	template<>
	auto Title::Update_<"Effect.Ambient.Raindrops">() -> void {
		auto emitRaindrops{
			[&, this] (Lumina::F32 hueFactor_) -> void {
				...
				for (Lumina::I32 i = 0; i < 32; ++i) {
					...
					Lumina::Particle p{};
					{
						...
						Lumina::F32 const y{
							RNDEngine() * Inv_0xFFFFFFFF * 10.0f +
							TitleEffect::ViewToWorld[3].Y()
						};
						Lumina::F32 const z{
	...
	template<>
	auto Title::Update_<"Effect.Ambient.Sparkle">() -> void {
		...
		Lumina::F32 const z{
			RNDEngine() * Inv_0xFFFFFFFF * 5.0f - 2.5f
		};
	...
	template<>
	auto Title::Update_<"RaindropParticle">(Lumina::Particle& p_) -> void {
		p_.Translate.X += p_.Velocity.X;
		p_.Translate.Y += p_.Velocity.Y;
		p_.Translate.Z += p_.Velocity.Z;

		p_.Rotate.Z = (RNDEngine() * Inv_0xFFFFFFFF - 0.5f) * 0.05f;

		p_.Life -= 1.0f;
	}

	template<>
	auto Title::Update_<"AmbientSparkleParticle">(Lumina::Particle& p_) -> void {
		...
		Lumina::F32 const factor{
			1.0f - (p_.Life - 30.0f) * (p_.Life - 30.0f) * TitleEffect::Inv_900
		};
